...
def cargar_clave_publica(archivo):
    with open(archivo, "rb") as f:               # ABRE EL ARCHIVO DE CLAVE PUBLICA EN MODO LECTURA BINARIA
        clave = RSA.import_key(f.read())         # LEE E IMPORTA LA CLAVE PUBLICA DEL ARCHIVO
...
cifra_publica = cargar_clave_publica("clave_publica.pem")   # CARGA LA CLAVE PUBLICA
cifra_privada = cargar_clave_privada("clave_privada.pem")   # CARGA LA CLAVE PRIVADA

# FUNCIONES DE RED Y COMUNICACION
...
def enviar_mensaje(cliente):
    while True:
        try:
            ...
            cliente.send(mensaje_cifrado.encode())          # ENVIA EL MENSAJE CIFRADO AL CLIENTE
...
def main():
    ip_servidor = input("Introduce la IP del servidor UC1: ") # SOLICITA AL USUARIO LA IP DE UC1
    puerto_servidor = int(input("Introduce el puerto del servidor UC1: ")) # SOLICITA AL USUARIO EL PUERTO DE UC1
    
    ip_cliente = input("Introduce la IP del cliente UC2: ")  # SOLICITA AL USUARIO LA IP DE UC2
    puerto_cliente = int(input("Introduce el puerto del cliente UC2: ")) # SOLICITA AL USUARIO EL PUERTO DE UC2
    ...
    while True:
        try:
            ...
            cliente_thread.start()                          # INICIA EL HILO

            while True:
                cliente_conectado, direccion = servidor.accept() # ACEPTA LA CONEXION
                cliente_thread = threading.Thread(target=manejar_cliente, args=(cliente_conectado, direccion)) # HILO PARA MANEJAR LA CONEXION CON EL CLIENTE
                cliente_thread.start()                      # INICIA EL HILO
